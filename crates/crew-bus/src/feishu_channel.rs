...
impl FeishuChannel {
    ...
    async fn parse_event(&self, envelope: &serde_json::Value) -> Option<InboundMessage> {
        ...
        if content.is_empty() && media.is_empty() {
            debug!(message_id, msg_type, "Feishu: empty content and media, skipping");
            return None;
        }

        info!(message_id, msg_type, media_count = media.len(), "Feishu: parsed event");
    ...
    async fn start_webhook(&self, inbound_tx: mpsc::Sender<InboundMessage>) -> Result<()> {
        ...
        async fn handle_webhook(
            ...
            let event_json = if let Some(encrypt_str) = body_json
                .get("encrypt")
                .and_then(|v| v.as_str())
            {
                if let Some(ref ek) = state.encrypt_key {
...
mod tests {
    ...
    #[test]
    fn test_with_mode() {
        let ch = FeishuChannel::new("id", "secret", vec![], Arc::new(AtomicBool::new(false)), "global", PathBuf::from("/tmp"))
            .with_mode("webhook")
