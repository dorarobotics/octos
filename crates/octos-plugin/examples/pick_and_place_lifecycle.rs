...
//! ## The Problem
//!
//! Industrial robots need a strict startup/shutdown sequence: check air pressure,
//! power on servo drives, home axes, verify sensors — in that exact order. If you
//! skip the preflight check and the gripper has no air, the robot drops a part on
//! the conveyor. If you skip homing, the arm doesn't know where it is and crashes
//! into the workcell wall on its first move.
//!
//! Shutdown is equally critical. Power off servos before parking the arm? It falls
//! under gravity. Skip the gripper release? It's still clamping a part when
//! maintenance tries to move it manually.
//!
//! **Before this feature:**
//! - Startup/shutdown is ad-hoc shell scripts or manual operator steps.
//! - No retry logic — if the servo driver fails once on power-on (common with
//!   older hardware), the whole startup fails and someone has to SSH in and retry.
//! - No critical/non-critical distinction — a flaky encoder check aborts the
//!   entire startup even though the robot can run fine without it.
//! - No emergency path — graceful shutdown takes 30 seconds; you need a 2-second
//!   path for e-stop situations.
//!
//! **After this feature:**
//! - `HardwareLifecycle` declares 5 ordered phases as data, not code.
//! - Each `LifecycleStep` has timeout, retries, and critical flag.
//! - `LifecycleExecutor` runs steps in order with automatic retry and timeout.
//! - Critical failures abort the phase; non-critical failures log and continue.
//! - Emergency shutdown is a separate fast path with short timeouts.
//!
//! ## Scenario
//!
//! A pick-and-place workcell with a 6-axis arm, pneumatic gripper, and conveyor.
//! The lifecycle covers: preflight checks -> initialization -> ready verification
//! -> normal operation -> graceful shutdown (or emergency stop).
//!
//! ```bash
...
async fn main() {
    ...
    //
    // Each phase is an ordered list of steps. The executor runs them top to
    // bottom. Notice the design choices in each phase:
    //
    // - PREFLIGHT: Check prerequisites. Retries on flaky checks (air supply
    //   sensor sometimes reads zero on first poll). Non-critical steps
    //   (conveyor encoder) don't abort the whole startup.
    //
    // - INIT: Bring hardware up in dependency order. Servo drives MUST succeed
    //   (critical=true, retries=2) because everything depends on them. Conveyor
    //   is non-critical — the arm can pick/place without it.
    //
    // - READY_CHECK: Final verification before handing control to the agent.
    //   All critical — if force sensor isn't zeroed, contact detection won't
    //   work and the robot could crush parts.
    //
    // - SHUTDOWN: Reverse of init. Park arm first (while servos still powered),
    //   then release gripper, then power off. Order matters!
    //
    // - EMERGENCY_SHUTDOWN: 2-second budget. Stop everything NOW. No retries,
    ...
    // ── Step 2: Run the normal startup sequence ──
    //
    // LifecycleExecutor::run_phase() handles the complexity:
    // - Runs steps in order
    ...
    let startup_phases: Vec<(&str, &[LifecycleStep])> = vec![
    ...
    for (name, steps) in &startup_phases {
    ...
    // ── Step 3: Normal graceful shutdown ──
    //
    // Called when the operator ends the session or the agent completes its task.
    // Steps run in reverse dependency order (park arm -> release gripper ->
    // stop conveyor -> power off servos).
    println!("╔══════════════════════════════════════════╗");
    ...
    // ── Step 4: Emergency shutdown (separate fast path) ──
    //
