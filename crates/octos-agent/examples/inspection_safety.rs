...
use octos_agent::{
    BlackBoxRecorder, HookEvent, RobotPayload, RobotPermissionPolicy, SafetyTier, WorkspaceBounds,
};
...
async fn main() -> eyre::Result<()> {
    ...
    let policy =
        RobotPermissionPolicy::new(SafetyTier::SafeMotion).with_workspace(workspace.clone());
    ...
    let tool_checks = [
        ...
        (
            ...
            "slow motion within bounds",
        ),
        (
            "joint_full_actuation",
    ...
    for (tool, tier, why) in &tool_checks {
        let result = match policy.authorize(tool, *tier) {
            Ok(()) => format!(
                "ALLOWED  (requires {tier}, session allows up to {})",
                policy.max_tier
            ),
    ...
    let test_points = [
        ...
        (
            ...
            0.0,
            0.4,
            "valve V-202 (outside X limit — in adjacent cell!)",
        ),
        (
            0.0,
            0.0,
            -0.1,
            "below table surface (impossible / collision)",
        ),
    ...
    println!(
        "  Force/torque: {:?} (Fz=15.8N — valve stuck?)",
        force_payload.force_torque
    );
    ...
    let events_explained = [
        (
            ...
            "Fires BEFORE any motion. Hook can DENY to prevent it.",
        ),
        (
            HookEvent::AfterMotion,
            "Fires AFTER motion completes. Log result, update state.",
        ),
        (
            HookEvent::ForceLimit,
            "Fires when force/torque exceeds limits. Trigger safe-hold.",
        ),
        (
            HookEvent::WorkspaceBoundary,
            "Fires when target is outside bounds. Auto-denied.",
        ),
        (
            HookEvent::EmergencyStop,
            "Fires on e-stop. All motion halts immediately.",
        ),
    ];
